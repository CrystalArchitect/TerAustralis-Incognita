...
**As of:** 2026-08-29 (cross-repo memory model restructured and merged to main via PR #129; memory-state model design hypothesis merged to main via PR #131; Starline Arsenal expanded 13 → 25 models, merged to main via PR #130; Starline Arsenal expanded 25 → 26 models + Etymology Stack methodology added, this session, not yet merged)
...
**Session 2026-08-29 (d, this branch, `claude/provenance-stack`, not yet merged):** Crystal asked "what's wrong with the etymology stuff?" (referring to another Claude Code session's fact-checking of etymology claims from YouTube source material, already on disk in [`../LANGUAGE-AS-PROGRAMMING.md`](../LANGUAGE-AS-PROGRAMMING.md)), then relayed a Grok-drafted "CrystalCore Etymology Stack" proposal — a provenance-tiered evidence system for word-origin claims — and asked for it to be incorporated into the Starline Arsenal in any overlapping categories. Two artifacts added: (1) [`../ETYMOLOGY-STACK.md`](../ETYMOLOGY-STACK.md) — a standalone methodology doc generalizing the ad hoc fact-checking already done in `LANGUAGE-AS-PROGRAMMING.md` into a reusable evidence-tier stack (A+ Attested through V Vision), with the four non-negotiable rules, two worked YAML evidence cards (government, abracadabra), and a 9-row summary table of that session's fact-checked claims. (2) `.claude/skills/starline-arsenal/models/26-provenance-stack.md` — new 26th model, "Provenance Stack" (The Deconstructors, register ARCHIVE + LOOM), generalizing the same method beyond etymology to any claim where source quality, not logical structure, is the actual question. `INDEX.md` and `SKILL.md` bumped 25 → 26 (version 2.0.0 → 2.1.0). Cross-reference sections added to the four models with genuine overlap — [First Principles](../../.claude/skills/starline-arsenal/models/01-first-principles.md) (evidenced-vs-assumed binary vs. graded tiers), [Circle of Competence](../../.claude/skills/starline-arsenal/models/18-circle-of-competence.md) (grades the judge vs. grades the source), [Inference](../../.claude/skills/starline-arsenal/models/19-inference.md) (sorts one reasoning step vs. sorts the sources feeding it), [Occam's Razor](../../.claude/skills/starline-arsenal/models/14-occams-razor.md) (a low-tier source is exactly the unearned complexity to cut) — each stating the differentiation, not just a link. **Label: same status as the 14–25 batch — Vision/drafted, not verified against Grok's own wording for either the Etymology Stack proposal or model 26; the existing open question in `../OPEN-QUESTIONS.md` about reconciling against Grok's actual text covers this addition too.** Crystal then asked to also cross-reference "the Crystal Equation" and "Roman numerals." Added: a worked evidence card for Roman numerals (I/V/X/L/C/D/M as letters reused as numbers — Tier A+/A, well-attested) and a "Letters as numbers" section distinguishing three things often conflated — the Roman-numeral fact (attested), the real historical practice of gematria/isopsephy (attested as a practice), and any specific numeric "hidden meaning" reading drawn from that practice, the Crystal Equation's own letter↔number system included (Vision tier — legitimate as personal/creative symbolism, explicitly not filed as linguistic-historical fact). The Crystal Equation's actual letter-number mapping is **not** reproduced in this repo — it was developed mostly in ChatGPT (per Crystal, correcting an earlier assumption in this same session that it was a Grok artifact) and was not supplied on disk; only a pointer noting its tier was added, per the rule against inventing canon.
