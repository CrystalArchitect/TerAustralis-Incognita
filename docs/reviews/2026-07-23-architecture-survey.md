...
1. ⚠️ **The docs repo describes a repo that no longer exists.** Its README, ADRs, and every agent-instruction file still assume `src/`, `tests/`, and `scripts/` live inside it. `git log` confirms those paths were *never* tracked there — the code has only ever lived in the sibling repo.
...
> 🔴 **The core problem.** Essentially every architecture document, ADR, and agent-instruction file in the docs repo — `README.md`, `AGENTS.md`, `docs/architecture/SystemMap.md`, all five files under `docs/agents/` — describes a single monorepo where code lives alongside docs. `git log --oneline --all -- src` against the docs repo returns **nothing**: not moved, not deleted, never present. The move to a second repo happened, but no document was ever updated to say so. The docs repo's own CI workflow still tries to run `compileall src tests` against directories that were never there.
