...
This repo's honesty discipline is real where it matters most: the three locked names are genuinely locked and unredefined, the Indigenous Data Sovereignty boundary is substantive rather than aspirational, and the retired edge-companion name has in fact been kept out of every live document — a rare thing for a fast-moving, AI-co-authored project to actually pull off. Set against that, the "Belt-Three" law is inconsistently defined between its own two canonical sources, "Songline" leaks into live documents as a component name (including a governance document actively proposing it as a *future* name), and roughly 3,500 lines of genuine importable ML training code sit inside the umbrella repo's mythos folder under a description that fits only a fraction of what's actually there. Three small textual defects (a corrupted word from an old find-replace, and an American spelling in the project's own style-setting document) are fixed directly in this PR; everything else is documented with evidence for a human to weigh.
...
### 1. 🔴 "Songline" is used as a component name in live governance documents — including one actively proposing it as a future name

The rule, stated in this project's own words: *"'Songline' is never a component name... Where Songlines appear in mythos or art they are honoured as cultural image, never claimed"* ([`Indigenous-Data-Sovereignty.md`](../governance/Indigenous-Data-Sovereignty.md), restated in [`mythos/NAMES.md`](../../mythos/NAMES.md)). Two distinct violations of it exist today, in live (non-archived) documents:

- **Most serious — a live, open governance document proposes it as a candidate name.** [`docs/OPEN-DECISIONS.md`](../OPEN-DECISIONS.md) lines 245–247, under "Naming Disambiguation: Starline" (status: Vision 🔮, "last verified 2026-07-23", i.e. still an open, unresolved question):
  ...
  This is not a historical record — it is a currently-open recommendation list that a future PR could act on literally as written, which would mint exactly the kind of component name the project's own Indigenous Data Sovereignty page forbids.

- **Historical record — a real pre-reorg repository was named `SonglineBus`, and four live documents repeat that name as the label for it:** [`docs/TIMELINE.md:51`](../TIMELINE.md), [`docs/governance/Project-Boundaries.md:117`](../governance/Project-Boundaries.md), [`docs/REPOSITORIES.md:244,252`](../REPOSITORIES.md), [`docs/ARCHITECTURE.md:180`](../ARCHITECTURE.md) — e.g. *"The Songline protocol pack (SonglineBus, original architecture before Starline Weaver)"*. Unlike the OPEN-DECISIONS case, this is stated as fact about what a retired repository was actually called, and Repository Principle 3 says history should be preserved, not deleted — so this one is genuinely harder to resolve cleanly, and is left as an open question below rather than edited.

**Recommendation:** Remove the `SonglineTransport` / `Songline Network` candidates from `OPEN-DECISIONS.md`'s naming-disambiguation list outright — an *open decision* document is exactly the place this rule should be enforced before anything is built, not after. Not fixed in this PR because choosing what replaces those two candidates is a naming decision the brief reserves for a human.

**❓ Open question:** Does the "Songline is never a component name" rule apply retroactively to *historical* facts about an already-renamed, already-retired repository (`SonglineBus`, tag `crystalcore-safe-2026-07-17`), or only to live/future naming? The current four documents describing that lineage read as factual record, not endorsement — but they do put "Songline" in a name-shaped position on the page. The maintainer who holds the actual repository history is better placed to decide whether to keep the name as historical record or rephrase around it (e.g. "the original, unnamed bus" or "the pre-Starline bus").
