...
`profiles/<name>/bridge_config.json` to load, and also which of Lumina's
own profiles (`apps/lumina/lumina_profiles/<name>/`) to give access
...
relative to `apps/lumina/` rather than trusting the relative helper —
...
| `message` | Leaves a note for the human | No — written to `profiles/<name>/messages.jsonl`, deliberately **not** folded into Lumina's memory automatically |
