...
| **Dreamline Train** | the traveller — what journeys the map, carrying memory from node to node | rides the peer-to-peer transport in [`src/crystal-core/starline/`](../src/crystal-core/starline/) — consent-gated, Noise-handshake memory exchange | Running, self-tested |
...
Starline rails (the `src/crystal-core/starline/` peer-to-peer network) to carry memory between
